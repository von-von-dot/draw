...
function computeFusionVisualMask(g){
  const o = _fv.off.getContext("2d");
  o.clearRect(0,0,canvas.width,canvas.height);
  // draw disks in fg (we threshold alpha later)
  o.fillStyle = "#000";
  const r = +els.radius.value;
  for(const p of state.points){
    o.beginPath();
  ...
  const m = _fv.mask.getContext("2d");
  m.clearRect(0,0,canvas.width,canvas.height);
  m.save();
  m.filter = `blur(${(+els.fusion.value).toFixed(2)}px)`;
  m.drawImage(_fv.off, 0, 0);
  m.restore();

  const w = g.inner.x1 - g.inner.x0;
  const h = g.inner.y1 - g.inner.y0;
  const img = m.getImageData(g.inner.x0, g.inner.y0, w, h);
  const data = img.data;
  const thr = isoToAlphaThreshold(+els.iso.value);
  ...
  m.putImageData(img, g.inner.x0, g.inner.y0);
  return _fv.mask;
}
