...
function computeFusionVisualMask(g){
  ...
  if(!computeFusionVisualMask._c){
    computeFusionVisualMask._c = document.createElement("canvas");
    computeFusionVisualMask._ctx = computeFusionVisualMask._c.getContext("2d", { willReadFrequently:true });
  }
  const oc = computeFusionVisualMask._c;
  const o = computeFusionVisualMask._ctx;
  ...
  // Draw blurred disks using shadowBlur (more reliable on iOS Safari than ctx.filter)
  o.save();
  o.translate(0.5,0.5);
  o.fillStyle = "#000";
  o.shadowColor = "#000";
  o.shadowBlur = fusion; // main control
  o.shadowOffsetX = 0;
  o.shadowOffsetY = 0;

  const r = +els.radius.value;

  // draw points (and their symmetries) as separate circles
  const pts = [];
  for(const p of state.points){
  ...
  // Threshold alpha & colorize with fg
  const img = o.getImageData(g.inner.x0, g.inner.y0, g.inner.w, g.inner.h);
  const data = img.data;
  ...
  o.putImageData(img, g.inner.x0, g.inner.y0);
